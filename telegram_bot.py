...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "💳 Pay with Flutterwave":
        ...
        await update.message.reply_text("Click below to pay with Flutterwave:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"User {user_id} approved and invited.")
...
async def flutterwave_webhook(request: Request, verif_hash: str = Header(None)):
    ...
    if data.get("status", "").lower() in ("successful", "success", "completed"):
        ...
        if tx_ref and tx_ref.startswith("tg_"):
            ...
            if telegram_id not in verified_users:  # avoid duplicates
